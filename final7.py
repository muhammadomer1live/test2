...
def evaluate_model_with_rouge_and_bleu(model, dataloader, word_to_idx, idx_to_word, progress_var, progress_label, rouge_label, bleu_label, start_time):
    model.eval()
    hypotheses = []
    references = []
    ...
    with torch.no_grad():
        for batch_idx, (images, captions) in enumerate(dataloader):
            ...
            for i in range(images.size(0)):
                ...
                references.append([ref])  # Wrap reference in a list
    ...
    # Compute BLEU scores
    bleu_score = compute_bleu_scores(hypotheses, [ref for refs in references for ref in refs])
    
    # Update the GUI with the ROUGE and BLEU scores
    rouge_label.config(text=f"ROUGE-1: {rouge_scores['rouge1']:.4f}, ROUGE-2: {rouge_scores['rouge2']:.4f}, ROUGE-L: {rouge_scores['rougeL']:.4f}")
    bleu_label.config(text=f"BLEU Score: {bleu_score:.4f}")
