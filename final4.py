...
transform = transforms.Compose([
    transforms.Resize((128, 128)),  # Resize to a smaller size for faster processing
...
def compute_rouge_scores(predictions, references):
    ...
    for pred, ref in zip(predictions, references):
        # Compute ROUGE scores for each predicted and reference caption
    ...
    # Compute average ROUGE scores across the dataset
...
def evaluate_model_with_rouge(model, dataset, word_to_idx, idx_to_word, max_length=20):
    model.eval()
    ...
    avg_rouge1, avg_rouge2, avg_rougeL = compute_rouge_scores(predictions, references)
    
    print(f"Average ROUGE-1: {avg_rouge1:.4f}")
    print(f"Average ROUGE-2: {avg_rouge2:.4f}")
    print(f"Average ROUGE-L: {avg_rougeL:.4f}")

# After training, evaluate the model with ROUGE
evaluate_model_with_rouge(nic_model, train_dataset, word_to_idx, idx_to_word)
