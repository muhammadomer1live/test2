...
def train_model_gui(model, train_loader, optimizer, criterion, num_epochs=10):
...
train_model_gui(model, train_loader, optim.Adam(model.parameters(), lr=0.001), nn.CrossEntropyLoss(), num_epochs=10)
...
def create_captioning_gui(model, word_to_idx, idx_to_word):
    root = tk.Tk()
    ...
    def select_image():
        ...
        if file_path:
            ...
            display_image.thumbnail((300, 300))  # Resize image for display
    ...
    def show_caption():
        if selected_image:
            caption = generate_caption(model, selected_image, word_to_idx, idx_to_word)
    ...
    generate_button = ttk.Button(frame, text="Generate Caption", command=show_caption, state=tk.DISABLED)
    generate_button.grid(row=2, column=0, pady=5)

    caption_label = ttk.Label(frame, wraplength=300)
    caption_label.grid(row=3, column=0, pady=5)

    root.mainloop()

# Make sure to call this function after training the model
create_captioning_gui(model, word_to_idx, idx_to_word)