...
def start_gui():
    ...
    def on_train_button_click():
        try:
            ...
            # Assuming DataLoader and training functions are defined
            dataloader = DataLoader(load_dataset(max_size=data_size), batch_size=4, collate_fn=lambda x: coco_collate_fn(x, word_to_idx))
            start_time = time.time()

            if selected_model == "NIC Model":
                train_and_validate_model(nic_model, dataloader, criterion, optimizer_nic, num_epochs, progress_var, progress_label, start_time, canvas)
                torch.save(nic_model.state_dict(), 'nic_model.pth')
            elif selected_model == "SCST Model":
                train_and_validate_model(scst_model, dataloader, criterion, optimizer_scst, num_epochs, progress_var, progress_label, start_time, canvas)
    ...
    bleu_label = ttk.Label(evaluation_tab, text="BLEU Score: N/A", font=("Helvetica", 12))
    bleu_label.pack(pady=5)

    def on_evaluate_button_click():
        try:
            selected_eval_model = eval_model_selection.get()
            dataloader = DataLoader(load_dataset(max_size=100), batch_size=4, collate_fn=lambda x: coco_collate_fn(x, word_to_idx))
            start_time = time.time()

            if selected_eval_model == "NIC Model":
                nic_model.load_state_dict(torch.load('nic_model.pth'))
                evaluate_model_with_rouge_and_bleu(nic_model, dataloader, word_to_idx, idx_to_word, progress_var_eval, progress_label_eval, rouge_label, bleu_label, start_time)
            elif selected_eval_model == "SCST Model":
                scst_model.load_state_dict(torch.load('scst_model.pth'))
                evaluate_model_with_rouge_and_bleu(scst_model, dataloader, word_to_idx, idx_to_word, progress_var_eval, progress_label_eval, rouge_label, bleu_label, start_time)
    ...
    captions_label = ttk.Label(test_tab, text="Generated Caption: N/A", font=("Helvetica", 12))
    captions_label.pack(pady=5)
