...
def train_and_validate_model(model, train_dataloader, val_dataloader, criterion, optimizer, num_epochs=5, patience=3):
    ...
    # Create a Tkinter window for progress bar
    root = tk.Tk()
    root.title("Training Progress")

    progress_var = tk.DoubleVar()
    progress_bar = ttk.Progressbar(root, variable=progress_var, maximum=num_epochs * len(train_dataloader))
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (images, captions) in enumerate(train_dataloader):
            ...
            total_batches = len(train_dataloader) * num_epochs
            current_batch = epoch * len(train_dataloader) + batch_idx + 1
            progress_var.set(current_batch)

            elapsed_time = time.time() - start_time
            batches_left = total_batches - current_batch
            estimated_time_remaining = (elapsed_time / current_batch) * batches_left

            time_label.config(text=f"Time Remaining: {time.strftime('%H:%M:%S', time.gmtime(estimated_time_remaining))}")
            root.update()
    ...
    root.destroy()  # Close the progress bar window when training is complete
...
def create_training_gui():
    ...
    window = tk.Tk()
    window.title("Model Training Progress")
    ...
    time_label = tk.Label(window, text="Time remaining: calculating...")
    time_label.pack(pady=10)

    # Create a label to show the current epoch/loss information
    status_label = tk.Label(window, text="Training in progress...")
    ...
    return window, progress_bar, time_label, status_label
